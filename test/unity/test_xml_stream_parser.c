...
void test_uniqueness_check_same_name_should_fail(void) {
    ...
    TEST_ASSERT_EQUAL_INT(1, uniqueness_check(s1, s2));
    // Should fail due to same name
    ezxml_free(root);
}
...
void test_uniqueness_check_different_output_streams_should_pass(void) {
    ...
    ezxml_t s1 = ezxml_child(root, "stream");
    ezxml_t s2 = ezxml_next(s1);
    TEST_ASSERT_EQUAL_INT(0, uniqueness_check(s1, s2));
    // Should succeed
    ezxml_free(root);
}

void test_uniqueness_check_input_streams_same_filename_should_pass(
    void
) {
    const char *xml = "<streams>"
    ...
    ezxml_t s2 = ezxml_next(s1);
    TEST_ASSERT_EQUAL_INT(0, uniqueness_check(s1, s2));
    // Input streams can share filenames
    ezxml_free(root);
}
...
void test_parse_xml_tag_name_leading_space(void) {
    ...
    parse_xml_tag_name(tag_buf + 1, tag_name);
    // skip leading space manually
    TEST_ASSERT_EQUAL_STRING("stream", tag_name);
}
...
void test_parse_xml_tag_simple_tag(void) {
    ...
    offset = parse_xml_tag(
        xml, strlen(xml), tag, &tag_len, &line, &start_line
    );
...
void test_parse_xml_tag_handles_newlines(void) {
    ...
    TEST_ASSERT_EQUAL(4, start_line);
    // opening tag starts after 3 newlines
}
...
void test_missing_name_attribute(void) {
    const char *xml =
            "<stream type=\"input\" filename_template=\"file.nc\" input_interval=\"0_01:00:00\"/>";
...
void test_missing_type_attribute(void) {
    const char *xml =
            "<stream name=\"foo\" filename_template=\"file.nc\" input_interval=\"0_01:00:00\"/>";
    ezxml_t stream = make_stream(xml);
    TEST_ASSERT_EQUAL_INT(1, attribute_check(stream));
    ezxml_free(stream);
}
...
void test_missing_filename_template(void) {
    const char *xml =
            "<stream name=\"foo\" type=\"input\" input_interval=\"0_01:00:00\"/>";
    ezxml_t stream = make_stream(xml);
    TEST_ASSERT_EQUAL_INT(1, attribute_check(stream));
    ezxml_free(stream);
}
...
void test_input_missing_input_interval(void) {
    const char *xml =
            "<stream name=\"foo\" type=\"input\" filename_template=\"file.nc\"/>";
    ezxml_t stream = make_stream(xml);
    TEST_ASSERT_EQUAL_INT(1, attribute_check(stream));
    ezxml_free(stream);
}
...
void test_output_missing_output_interval(void) {
    const char *xml =
            "<stream name=\"foo\" type=\"output\" filename_template=\"file.nc\"/>";
    ezxml_t stream = make_stream(xml);
    TEST_ASSERT_EQUAL_INT(1, attribute_check(stream));
    ezxml_free(stream);
}
...
void test_illegal_filename_variable(void) {
    const char *xml =
            "<stream name=\"foo\" type=\"input\" filename_template=\"file_$X.nc\" input_interval=\"0_01:00:00\"/>";
    ezxml_t stream = make_stream(xml);
    TEST_ASSERT_EQUAL_INT(1, attribute_check(stream));
    ezxml_free(stream);
}
...
void test_valid_input_stream(void) {
    const char *xml =
            "<stream name=\"foo\" type=\"input\" filename_template=\"file_$Y.nc\" input_interval=\"0_01:00:00\"/>";
    ezxml_t stream = make_stream(xml);
...
void test_valid_output_stream(void) {
    const char *xml =
            "<stream name=\"bar\" type=\"output\" filename_template=\"out_$h.nc\" output_interval=\"0_03:00:00\"/>";
    ezxml_t stream = make_stream(xml);
    TEST_ASSERT_EQUAL_INT(0, attribute_check(stream));
    ezxml_free(stream);
}
...
void test_immutable_stream_with_variable(void) {
    const char *xml = "<streams>"
            ...
            "    <var name=\"temp\"/>" "  </immutable_stream>"
            "</streams>";
    ezxml_t root = make_streams(xml);
    TEST_ASSERT_NOT_EQUAL(0, check_streams(root));
    ezxml_free(root);
...
void test_stream_with_unreadable_file_reference(void) {
    const char *xml = "<streams>"
            ...
            "    <file name=\"nonexistent.yaml\"/>" "  </stream>"
            "</streams>";
    ezxml_t root = make_streams(xml);
    TEST_ASSERT_NOT_EQUAL(0, check_streams(root));
    ezxml_free(root);
...
void test_valid_interval_reference(void) {
    ...
    int rc = extract_stream_interval(
        "stream:A:input_interval", "input_interval", &interval2, "B",
        root
    );
...
void test_self_reference_should_fail(void) {
    ...
    const char *interval2 = NULL;
    int rc = extract_stream_interval(
        "stream:A:input_interval", "input_interval", &interval2, "A",
        root
    );
...
void test_invalid_attribute_reference(void) {
    ...
    const char *interval2 = NULL;
    int rc = extract_stream_interval(
        "stream:A:bad_attr", "input_interval", &interval2, "B", root
    );
    TEST_ASSERT_NOT_EQUAL(0, rc);
    TEST_ASSERT_NULL(interval2);
...
void test_reference_to_undefined_stream(void) {
    ...
    const char *interval2 = NULL;
    int rc = extract_stream_interval(
        "stream:Missing:input_interval", "input_interval", &interval2,
        "B", root
    );
    TEST_ASSERT_NOT_EQUAL(0, rc);
    TEST_ASSERT_NULL(interval2);
...
void test_unexpandable_interval_should_fail(void) {
    ...
    const char *interval2 = NULL;
    int rc = extract_stream_interval(
        "stream:B:output_interval", "input_interval", &interval2, "A",
        root
    );
    TEST_ASSERT_NOT_EQUAL(0, rc);
    TEST_ASSERT_NULL(interval2);
...
int main(void) {
    ...
    RUN_TEST(
        test_uniqueness_check_different_output_streams_should_pass
    );
    RUN_TEST(
        test_uniqueness_check_input_streams_same_filename_should_pass
    );
